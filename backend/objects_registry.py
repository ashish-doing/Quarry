...
class RegisteredObject:
    ...
    description: Optional[str] = None   # physical description (shape/color), e.g.
                                          # "blue cylinder" -- purely for humans: helps
                                          # register_mission_objects.py's operator find
                                          # the right photos, and can be shown to
                                          # spectators alongside the OSNet/OCR read so
                                          # they know what they're actually looking for
...
def description_for(target_name: str, objects: List[RegisteredObject]) -> Optional[str]:
    """Physical description (shape/color) for a registered target, e.g.
    "blue cylinder" -- purely informational, shown alongside OSNet/OCR
    results so spectators know what the object is supposed to look like."""
    for o in objects:
        if o.target_name == target_name:
